...
def display_product(result):
    ...
    if "stock_status" in product:
        ...
        if status == ct.STOCK_STATUS_LOW:
            st.warning(f"{ct.STOCK_ICON_LOW} {ct.STOCK_MESSAGE_LOW}")
        elif status == ct.STOCK_STATUS_NONE:
            st.error(f"{ct.STOCK_ICON_NONE} {ct.STOCK_MESSAGE_NONE}")
